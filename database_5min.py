"""
database_5min.py - 5-minute trading database handler with technical indicators
...
class Database5Min:
    """SQLite database handler for 5-minute trading data"""
    ...
    def _create_table(self):
        ...
        self.conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_timestamp_5min 
            ON candles_5min(timestamp DESC)
        """)
    ...
    def get_candle_count(self) -> int:
        ...
        cursor = self.conn.execute("SELECT COUNT(*) FROM candles_5min")
    ...
    def add_candle(self, candle_data: Dict):
        ...
        # Check if exists
        cursor = self.conn.execute(
            "SELECT 1 FROM candles_5min WHERE timestamp = ?", (timestamp,)
        )
        if cursor.fetchone():
            return
