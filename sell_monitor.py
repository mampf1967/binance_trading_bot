"""
sell_monitor.py - Sell decision monitoring

Monitors active positions and determines sell points using:
- Standard mode: +3% TP, -3% SL
- Recovery mode: +0.25% TP after 30 candles stagnation
- Time limit: Exit after 60 candles
...
class SellMonitor:
    """Monitors position and determines sell points"""
    ...
    def __init__(self, log_writer, symbol_base: str, position_manager=None,
        ...
        # Configuration
        self.tp_percent_orig = config.SELL_MONITOR['tp_percent_orig']
        self.sl_percent_orig = config.SELL_MONITOR['sl_percent_orig']
        self.recovery_tp = config.SELL_MONITOR['recovery_tp']
        self.spike_trigger = config.SELL_MONITOR['spike_trigger']
        self.stagnation_candles = config.SELL_MONITOR['stagnation_candles']
        self.max_candles = config.SELL_MONITOR['max_candles']
    ...
    def start_monitoring(self, entry_price: float, entry_time: str,
        ...
        if config.SELL_MONITOR.get('adaptive_stop_loss', False):
            analyzer = self._get_candle_analyzer()
            if analyzer:
                sl_multiplier = config.SELL_MONITOR.get('sl_bearish_multiplier', 2.0)
                min_sl = config.SELL_MONITOR.get('min_sl_percent', 2.0)
                max_sl = config.SELL_MONITOR.get('max_sl_percent', 5.0)
                ...
                self.sl_percent = config.SELL_MONITOR.get('sl_percent_orig', 3.0)
    ...
    def process_candle(self, candle: Dict) -> Optional[str]:
        ...
        # Update position PnL
        if self.position_manager:
            self.position_manager.update_pnl(self.symbol_base, '1min', close)
        
        # NEW: Check for danger-sized bearish (add before SL check)
        if config.SELL_MONITOR.get('auto_exit_large_bearish', False):
            open_price = float(candle.get('open', 0))
            close_price = float(candle.get('close', 0))
            
            # Only check bearish candles when in minimum profit
            if close_price < open_price:
                min_profit = config.SELL_MONITOR.get('exit_min_profit', 0.5)
                
                if pnl_percent >= min_profit:
                    analyzer = self._get_candle_analyzer()
                    if analyzer:
                        threshold = config.SELL_MONITOR.get('exit_bearish_threshold', 2.0)
        ...
        if config.SELL_MONITOR.get('recovery_mode_enabled', True) and self.recovery_mode:
        ...
        if self.entry_candle_count == self.stagnation_candles:
            quick_exit_threshold = config.SELL_MONITOR.get('quick_exit_threshold', 0.25)
            
            if config.SELL_MONITOR.get('quick_exit_enabled', True) and pnl_percent >= quick_exit_threshold:
                return self._execute_sell(candle, "QUICK_EXIT", pnl_percent, close)
            elif config.SELL_MONITOR.get('recovery_mode_enabled', True):
    ...
    def _execute_sell(self, candle: Dict, reason: str, pnl_percent: float, price: float) -> str:
        ...
        if self.cooldown_tracker and config.TRADING['cooldown_enabled']:
            self.cooldown_tracker.start_cooldown(self.symbol_base, '1min')
            cooldown_candles = config.TRADING['cooldown_1min_candles']
